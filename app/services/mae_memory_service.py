...
def find_approved_memory(question: str, limit: int = 4) -> list[dict]:
        ...
        with AnalyticsRepository() as repository:
            ...
            rows = repository.fetchall(
                ...
                SELECT
                    ...
                    approved_at,
                    approved_by
                FROM lcdash_analytics.mae_memory
            ...
            if selected:
                repository._execute(
                    """
                    UPDATE lcdash_analytics.mae_memory
    ...
    *,
    ...
    if source_interaction_id:
        try:
            ...
            raise ValueError("Invalid source interaction identifier.") from exc
    try:
        with AnalyticsRepository() as repository:
            repository.initialize_schema()
            row = repository.fetchone(
                """
                INSERT INTO lcdash_analytics.mae_memory (
                    created_by,
                    title,
                    trigger_text,
                    guidance,
    ...
    *,
    ...
    try:
        with AnalyticsRepository() as repository:
            ...
            repository._commit()
        if not row:
            return {"saved": False, "message": "Memory candidate not found."}
        return {"saved": True, "memory_id": int(row[0]), "status": row[1]}
    except AnalyticsDatabaseError as exc:
        return {"saved": False, "message": str(exc)}


def list_memory_items(limit: int = 200) -> list[dict]:
    try:
        with AnalyticsRepository() as repository:
            repository.initialize_schema()
            rows = repository.fetchall(
