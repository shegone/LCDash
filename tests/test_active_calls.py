...
class ActiveCallsPageTests(unittest.TestCase):
    ...
    def test_dashboard_uses_background_snapshot_refresh(self, snapshot_mock):
        ...
        self.assertIn('id="dashboard-refresh-button"', response.text)
        self.assertIn('id="dashboard-last-success-age"', response.text)
    ...
    def test_dashboard_refresh_script_keeps_last_known_data(self):
        ...
        self.assertIn('"Dashboard Connection Issue"', response.text)
        self.assertIn('"Updated just now"', response.text)
