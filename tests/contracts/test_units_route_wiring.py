"""Import-safe trusted-context wiring contract for the units API."""
...
class UnitsRouteWiringTests(unittest.TestCase):
    def setUp(self):
        self.blockers = [
            patch.object(
                socket.socket,
    ...
    def test_units_route_uses_trusted_dependency_and_exact_forwarding(self):
        argument_names = [argument.arg for argument in self.handler.args.args]
        self.assertEqual(argument_names, ["response", "tenant_context"])
